"""Locate all one bits using only range-existence queries.

Question:
    Locate all one bits using only range-existence queries.

Approach:
    Recursively discard zero ranges or binary-search each next one.

Complexity:
    O(K log M) queries and O(log M) recursion space.
