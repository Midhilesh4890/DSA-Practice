"""Answer address queries whose fields may be wildcards.

Question:
    Answer address queries whose fields may be wildcards.

Approach:
    Precompute every wildcard projection of each fixed-width address in a hash set.

Complexity:
    O(N*2^F*F) preprocessing, O(F) query time, and O(N*2^F*F) space.
...
# addressList=[(1,"A","AZ","AZZ"),(2,"B","AZ","ADD"),(1,"B","AZ","AZZ"),
