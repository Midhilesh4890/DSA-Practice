"""Store versioned JSON objects compactly and retrieve a version.

Question:
    Store versioned JSON objects compactly and retrieve a version.

Approach:
    Share key paths in a trie and record only versioned values at nodes.

Complexity:
    O(P) insert/read time and O(P*V) worst-case space.
