"""Simulate alternating left/right jumps and detect infinite play.

Question:
    Simulate alternating left/right jumps and detect infinite play.

Approach:
    Track normalized array, position, and parity states during deterministic simulation.

Complexity:
    O(T*N) time and space for T distinct states.
...
def game_end_position(A, S, X):
    ...
    while True:
        ...
        # Values may all increase by the same amount on every cycle, so raw
        # values do not repeat. Normalize to detect translation-equivalent
        # states such as [2, 1], start=1, increment=2.
        minimum = min(A)
        state = (tuple(value - minimum for value in A), current, move % 2)
