"""Connect all grid cities with a minimum road network.

Question:
    Connect all grid cities with a minimum road network.

Approach:
    BFS creates pairwise city paths and Kruskal selects a spanning set.

Complexity:
    O(C*R*G + C^2 log C) time and O(C*G) space for G grid cells.
